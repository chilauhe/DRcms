...
namespace DRcms.BLL
{
    ...
    public partial class site_channel
    {
        ...
        public bool Exists(string name)
        {
            ...
            if (DirPathExists(sysConfig.webpath + "/" + Constants.DIRECTORY_REWRITE_ASPX + "/", name))
            {
                return true;
            }
            //与存在的频道名称是否同名
        ...
        {
            Dictionary<int, string> dic = CacheHelper.Get<Dictionary<int, string>>(Constants.CACHE_SITE_CHANNEL_LIST);//从缓存取出
            //如果缓存已过期则从数据库里面取出
            if (dic == null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    CacheHelper.Insert(Constants.CACHE_SITE_CHANNEL_LIST, dic, 10);//重新写入缓存
                }
            }
            return dic;
        }
