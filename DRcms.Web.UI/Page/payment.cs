...
namespace DRcms.Web.UI.Page
{
    public partial class payment : Web.UI.BasePage
    {
        ...
        {
            //取得处事类型
            action = Common.Request.GetString("action");
            order_no = Common.Request.GetString("order_no");
            if (order_no.ToUpper().StartsWith("R")) //充值订单
            {
                order_type = Enums.AmountTypeEnum.Recharge.ToString().ToLower();
            }
            else if (order_no.ToUpper().StartsWith("B")) //商品订单
            {
                order_type = Enums.AmountTypeEnum.BuyGoods.ToString().ToLower();
            }
