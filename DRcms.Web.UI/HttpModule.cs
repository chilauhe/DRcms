...
namespace DRcms.Web.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            if (IsDirExist(Constants.CACHE_SITE_DIRECTORY, siteConfig.webpath, siteConfig.webpath, requestPath))
            {
                return;
            }
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls)
                {
                    ...
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        context.RewritePath(string.Format("{0}{1}/{2}{3}",
                            siteConfig.webpath, Constants.DIRECTORY_REWRITE_ASPX, sitePath, requestPage));
                    }
                }
                #endregion
            }
            else
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls)
                {
                        ...
                        Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        //映射到站点目录
                        context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                            siteConfig.webpath, Constants.DIRECTORY_REWRITE_ASPX, sitePath, model.page));
                        return;
                    }
                    //遍历URL字典的子节点
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPage, string.Format("^/{0}$", item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            ...
                            {
                                context.RewritePath(siteConfig.webpath + Constants.DIRECTORY_REWRITE_HTML + "/" + sitePath +
                            ...
                            else //其它
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, Constants.DIRECTORY_REWRITE_ASPX, sitePath, model.page), string.Empty, queryString);
                                return;
                            }
    ...
    public class SiteUrls
    {
        ...
        {
            SiteUrls _cache = CacheHelper.Get<SiteUrls>(Constants.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(Constants.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Utils.GetXmlMapPath(Constants.FILE_URL_XML_CONFING));
                    instance = CacheHelper.Get<SiteUrls>(Constants.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
        }
    ...
    public class SiteDomains
    {
        ...
        {
            SiteDomains _cache = CacheHelper.Get<SiteDomains>(Constants.CACHE_SITE_HTTP_DOMAIN);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(Constants.CACHE_SITE_HTTP_DOMAIN, new SiteDomains(), 10);
                    instance = CacheHelper.Get<SiteDomains>(Constants.CACHE_SITE_HTTP_DOMAIN);
                }
            }
            return instance;
        }
