                        ...
                        if (oldModel.name.ToLower() != model.name.ToLower() || oldModel.data_type.ToLower() != model.data_type.ToLower())
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    int rowsCount = Convert.ToInt32(DbHelperSQL.GetSingle(conn, trans, "select count(1) from syscolumns where id=object_id('" + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + "') and name='" + oldModel.name + "'"));
                                    if (rowsCount > 0)
                                    {
                                        //修改列数据类型
                                        if (oldModel.data_type.ToLower() != model.data_type.ToLower())
                                        {
                                            DbHelperSQL.ExecuteSql(conn, trans, "alter table " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + " alter column " + oldModel.name + " " + model.data_type);
                                        }
                                        //修改列名
                                        if (oldModel.name.ToLower() != model.name.ToLower())
                                        {
                                            DbHelperSQL.ExecuteSql(conn, trans, "exec sp_rename '" + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + "." + oldModel.name + "','" + model.name + "','column'");
                                        }
                                    }
                                }
                            }
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                //检查有无该频道数据表和列
                                int rowsCount = Convert.ToInt32(DbHelperSQL.GetSingle(conn, trans, "select count(1) from syscolumns where id=object_id('" + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + "') and name='" + model.name + "'"));
                                if (rowsCount > 0)
                                {
                                    //删除频道数据表一列
                                    DbHelperSQL.ExecuteSql(conn, trans, "alter table " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + " drop column " + model.name);
                                }
                            }
                        }
