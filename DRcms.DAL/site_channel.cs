...
namespace DRcms.DAL
{
    ...
    public partial class site_channel
    {
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql2.Append("drop table " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channelName);
        ...
        private void FieldAdd(SqlConnection conn, SqlTransaction trans, Model.site_channel model)
        {
            ...
            strSql2.Append("CREATE TABLE " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + model.name + "(\r\n");
        ...
        private void FieldUpdate(SqlConnection conn, SqlTransaction trans, Model.site_channel newModel, Model.site_channel oldModel)
        {
            if (newModel.channel_fields != null)
            {
                ...
                if (newFieldIds.Length > 0)
                {
                    ...
                    {
                        DbHelperSQL.ExecuteSql(conn, trans, "alter table " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + oldModel.name + " add " + dr["name"].ToString() + " " + dr["data_type"].ToString());
                    }
                }
            ...
            {
                DbHelperSQL.ExecuteSql(conn, trans, "exec sp_rename '" + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + oldModel.name + "', '" + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + newModel.name + "'");
            }
        }
        ...
        private void FieldDelete(SqlConnection conn, SqlTransaction trans, Model.site_channel newModel, Model.site_channel oldModel)
        {
            ...
            if (fieldIds.Length > 0)
            {
                ...
                {
                    //删除频道数据表列
                    DbHelperSQL.ExecuteSql(conn, trans, "alter table " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + oldModel.name + " drop column " + dr["name"].ToString());
                }
            }
        }
