...
namespace DRcms.DAL
{
	...
	public partial class article
	{
        ...
        public bool Exists(string channel_name, int article_id)
        {
            ...
            strSql.Append("select count(1) from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("insert into " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channelName + "(");
                        //主表字段信息
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        PropertyInfo[] pros = model.GetType().GetProperties();
                        List<SqlParameter> paras = new List<SqlParameter>();
                        strSql.Append("update  " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channelName + " set ");
                        //主表字段信息
                        foreach (PropertyInfo pi in pros)
                        {
                            //如果不是主键或List<T>则追加sql字符串
		...
		public bool Delete(string channel_name, int channel_id, int article_id)
		{
            ...
            //删除主表
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 * from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = article_id;
        ...
        public DataSet GetList(string channel_name, int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" * ");
            strSql.Append(" FROM " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            if (category_id > 0)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 title from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 content from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 img_url from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 click from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name + " set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        public DataSet ArticleList(string channel_name, int category_id, Dictionary<string, string> dicSpecIds, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            ...
            if (!string.IsNullOrEmpty(specWhere))
            {
                ...
                strSql.Append("select A.id from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + channel_name + " as A," + databaseprefix + "article_goods as B");
        ...
        public DataSet ArticleSearch(int site_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    strSql.Append(" from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dt.Rows[i]["name"].ToString());
                    strSql.Append(" where datediff(d,add_time,getdate())>=0");
                    if (strWhere.Trim() != "")
                    {
                        strSql.Append(" and " + strWhere);
        ...
        public DataSet ArticleSearch(int site_id, string tags, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    strSql.Append(" from " + databaseprefix + Constants.TABLE_CHANNEL_ARTICLE + dt.Rows[i]["name"].ToString() + " as A INNER JOIN (");
                    strSql.Append("select R.channel_id,R.article_id");
                    strSql.Append(" from dr_article_tags_relation as R INNER JOIN dr_article_tags as S ON R.tag_id=S.id and S.title='" + tags + "'");
