...
namespace DRcms.Web.admin.users
{
    public partial class site_oauth_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_oauth", Enums.ActionEnum.View.ToString()); //检查权限
                SiteBind(); //绑定站点
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("site_oauth_list_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("site_oauth_list_page_size", "DRcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_oauth", Enums.ActionEnum.Edit.ToString()); //检查权限
            BLL.site_oauth bll = new BLL.site_oauth();
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "保存OAuth列表排序"); //记录日志
        ...
        {
            ChkAdminLevel("user_oauth", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.site_oauth bll = new BLL.site_oauth();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除OAuth列表成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
