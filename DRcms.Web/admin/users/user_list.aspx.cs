...
namespace DRcms.Web.admin.users
{
    public partial class user_list : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("user_list", Enums.ActionEnum.View.ToString()); //检查权限
            this.site_id = Common.Request.GetQueryInt("site_id");
            this.group_id = Common.Request.GetQueryInt("group_id");
            this.keywords = Common.Request.GetQueryString("keywords");
            this.start_time = Common.Request.GetQueryString("start_time");
            this.end_time = Common.Request.GetQueryString("end_time");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_list_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_list_page_size", "DRcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("user_list.aspx", "site_id={0}&group_id={1}&start_time={2}&end_time={3}&keywords={4}",
        ...
        {
            ChkAdminLevel("user_list", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.users bll = new BLL.users();
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除用户" + sucCount + "条，失败" + errorCount + "条"); //记录日志
