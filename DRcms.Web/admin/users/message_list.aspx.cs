...
namespace DRcms.Web.admin.users
{
    public partial class message_list : Web.UI.ManagePage
    {
        ...
        {
            this.type_id = Common.Request.GetQueryInt("type_id");
            this.keywords = Common.Request.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_message", Enums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("message_list_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("message_list_page_size", "DRcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_message", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_message bll = new BLL.user_message();
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除站内短消息成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
