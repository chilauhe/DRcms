...
namespace DRcms.Web.admin.users
{
    public partial class recharge_edit : Web.UI.ManagePage
    {
        ...
        {
            this.username = Common.Request.GetQueryString("username"); //获得用户名
            if (!Page.IsPostBack)
            {
        ...
        private bool DoAdd()
        {
            ...
            {
                AddAdminLog(Enums.ActionEnum.Add.ToString(), "给会员：" + model.user_name + "，充值:" + model.amount + "元"); //记录日志
                result = true;
            }
            return result;
        }
        ...
        {
            ChkAdminLevel("user_recharge_log", Enums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
