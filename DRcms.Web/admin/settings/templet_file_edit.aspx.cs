...
namespace DRcms.Web.admin.settings
{
    public partial class templet_file_edit : Web.UI.ManagePage
    {
        ...
        {
            pathName = Common.Request.GetQueryString("path");
            fileName = Common.Request.GetQueryString("filename");
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_site_templet", Enums.ActionEnum.View.ToString()); //检查权限
                ShowInfo(filePath);
            }
        }
        ...
        {
            ChkAdminLevel("sys_site_templet", Enums.ActionEnum.Edit.ToString()); //检查权限
            if (!File.Exists(filePath))
            {
                JscriptMsg("模板文件不存在！", "back");
                return;
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "修改模板文件:" + this.fileName);//记录日志
