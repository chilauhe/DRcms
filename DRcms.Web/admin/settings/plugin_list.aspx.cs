...
namespace DRcms.Web.admin.settings
{
    public partial class plugin_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_plugin_config", Enums.ActionEnum.View.ToString()); //检查权限
                RptBind();
            }
        }
        ...
        private void RemoveTemplates(string dirName)
        {
            ...
            XmlNodeList xnList = XmlHelper.ReadNodes(fullPath + Constants.FILE_PLUGIN_XML_CONFING, "plugin/urls");
            if (xnList.Count > 0)
            {
                    ...
                    if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite" && xe.Attributes["page"] != null)
                    {
                        if (xe.Attributes["name"] != null && xe.Attributes["type"] != null)
                        {
                            ...
                            if (tempPath.Exists)
                            {
                                ...
                                {
                                    //删除对应站点下的aspx文件
                                    FileHelper.DeleteFile(sysConfig.webpath + Constants.DIRECTORY_REWRITE_ASPX + "/" + dirInfo.Name + "/" + xe.Attributes["page"].Value);
                                }
                            }
                        }
                    }
        ...
        {
            ChkAdminLevel("sys_plugin_config", Enums.ActionEnum.Instal.ToString()); //检查权限
            //插件目录
                ...
                if (cb.Checked)
                {
                    ...
                    if (model.isload == 0)
                    {
                        ...
                        if (tempPath.Exists)
                        {
                            ...
                            {
                                //如果该站点目录存在则生成模板
                                if (Directory.Exists(Utils.GetMapPath(sysConfig.webpath + Constants.DIRECTORY_REWRITE_ASPX + "/" + dirInfo.Name + "/")))
                                {
            ...
            AddAdminLog(Enums.ActionEnum.Instal.ToString(), "安装插件"); //记录日志
        ...
        {
            ChkAdminLevel("sys_plugin_config", Enums.ActionEnum.UnLoad.ToString()); //检查权限
            //插件目录
            string pluginPath = Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(Enums.ActionEnum.UnLoad.ToString(), "卸载插件"); //记录日志
        ...
        {
            ChkAdminLevel("sys_plugin_config", Enums.ActionEnum.Build.ToString()); //检查权限
            //插件目录
            string pluginPath = Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(Enums.ActionEnum.Build.ToString(), "生成插件模板"); //记录日志
