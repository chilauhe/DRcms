...
namespace DRcms.Web.admin.settings
{
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_navigation", Enums.ActionEnum.View.ToString()); //检查权限
                TreeBind(Enums.NavigationEnum.System.ToString()); //绑定导航菜单
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.nav_type = Enums.NavigationEnum.System.ToString();
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(Enums.ActionEnum.Add.ToString(), "添加导航菜单:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(Enums.ActionEnum.Add.ToString(), "修改导航菜单:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Enums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_navigation", Enums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("sys_navigation", Enums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
