    ...
    public partial class url_rewrite_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == Enums.ActionEnum.Edit.ToString())
            {
                ...
                this.urlName = Common.Request.GetQueryString("name");
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_url_rewrite", Enums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定频道
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model))
            {
                AddAdminLog(Enums.ActionEnum.Add.ToString(), "添加URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(string _urlName)
        {
            ...
            if (bll.Edit(model))
            {
                AddAdminLog(Enums.ActionEnum.Edit.ToString(), "修改URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Enums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_url_rewrite", Enums.ActionEnum.Edit.ToString()); //检查权限
            ...
            {
                ChkAdminLevel("sys_url_rewrite", Enums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
