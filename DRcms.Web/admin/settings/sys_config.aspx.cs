...
namespace DRcms.Web.admin.settings
{
    public partial class sys_config : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_config", Enums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("sys_config", Enums.ActionEnum.Edit.ToString()); //检查权限
            BLL.sysconfig bll = new BLL.sysconfig();
            Model.sysconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(Enums.ActionEnum.Edit.ToString(), "修改系统配置信息"); //记录日志
