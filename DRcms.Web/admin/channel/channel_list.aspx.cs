...
namespace DRcms.Web.admin.channel
{
    public partial class channel_list : Web.UI.ManagePage
    {
        ...
        {
            this.site_id = Common.Request.GetQueryInt("site_id");
            this.keywords = Common.Request.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_channel_manage", Enums.ActionEnum.View.ToString()); //检查权限
                SiteBind(); //绑定站点
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("channel_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("channel_page_size", "DRcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("sys_channel_manage", Enums.ActionEnum.Edit.ToString()); //检查权限
            BLL.site_channel bll = new BLL.site_channel();
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "保存频道排序");//记录日志
        ...
        {
            ChkAdminLevel("sys_channel_manage", Enums.ActionEnum.Delete.ToString());//检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.site_channel bll = new BLL.site_channel();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除频道成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            CacheHelper.Remove(Constants.CACHE_SITE_CHANNEL_LIST); //更新一下频道的缓存
