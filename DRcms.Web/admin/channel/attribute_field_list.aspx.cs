...
namespace DRcms.Web.admin.channel
{
    public partial class attribute_field_list : Web.UI.ManagePage
    {
        ...
        {
            this.control_type = Common.Request.GetQueryString("control_type");
            this.keywords = Common.Request.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_channel_field", Enums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("attribute_field_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("attribute_field_page_size", "DRcmsPage", _pagesize.ToString(), 43200);
                }
            }
        ...
        {
            ChkAdminLevel("sys_channel_field", Enums.ActionEnum.Edit.ToString()); //检查权限
            BLL.article_attribute_field bll = new BLL.article_attribute_field();
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "保存扩展字段排序"); //记录日志
        ...
        {
            ChkAdminLevel("sys_channel_field", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0; //成功数量
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除扩展字段成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
