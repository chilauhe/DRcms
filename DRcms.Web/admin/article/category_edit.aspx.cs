...
namespace DRcms.Web.admin.article
{
    public partial class category_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + channelModel.name + "_category", Enums.ActionEnum.View.ToString()); //检查权限
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(Enums.ActionEnum.Add.ToString(), "添加" + channelModel.name + "频道栏目分类:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(Enums.ActionEnum.Edit.ToString(), "修改" + channelModel.name + "频道栏目分类:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Enums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + channelModel.name + "_category", Enums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("channel_" + channelModel.name + "_category", Enums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
