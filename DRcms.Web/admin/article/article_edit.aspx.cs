...
namespace DRcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        {
            string _action = Common.Request.GetQueryString("action");
            ...
            //如果是编辑或复制则检查信息是否存在
            if (_action == Enums.ActionEnum.Edit.ToString())
            {
                this.action = _action;//修改或复制类型
                this.id = Common.Request.GetQueryInt("id");
                if (this.id == 0)
                {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + this.channelModel.name + "_list", Enums.ActionEnum.View.ToString()); //检查权限
                ShowSysField(this.channel_id); //显示相应的默认控件
                GroupBind(string.Empty); //绑定用户组
                TreeBind(this.channel_id); //绑定类别
                if (action == Enums.ActionEnum.Edit.ToString() || action == Enums.ActionEnum.Copy.ToString()) //修改
                {
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(Enums.ActionEnum.Add.ToString(), "添加" + this.channelModel.title + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(Enums.ActionEnum.Edit.ToString(), "修改" + this.channelModel.title + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("channel_" + this.channelModel.name + "_list", Enums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("channel_" + this.channelModel.name + "_list", Enums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
