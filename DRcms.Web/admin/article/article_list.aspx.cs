...
namespace DRcms.Web.admin.article
{
    public partial class article_list : Web.UI.ManagePage
    {
        ...
        {
            this.channel_id = Common.Request.GetQueryInt("channel_id");
            this.category_id = Common.Request.GetQueryInt("category_id");
            this.keywords = Common.Request.GetQueryString("keywords");
            this.property = Common.Request.GetQueryString("property");
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", Enums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            this.page = Common.Request.GetQueryInt("page", 1);
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("article_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_list", Enums.ActionEnum.Edit.ToString()); //检查权限
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("article_page_size", "DRcmsPage", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("article_list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "保存" + this.channel_name + "频道内容排序"); //记录日志
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_list", Enums.ActionEnum.Audit.ToString()); //检查权限
            BLL.article bll = new BLL.article();
            Repeater rptList = new Repeater();
            switch (this.prolistview)
            {
            ...
            AddAdminLog(Enums.ActionEnum.Audit.ToString(), "审核" + this.channel_name + "频道内容信息"); //记录日志
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_list", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0; //成功数量
            ...
            AddAdminLog(Enums.ActionEnum.Edit.ToString(), "删除" + this.channel_name + "频道内容成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
