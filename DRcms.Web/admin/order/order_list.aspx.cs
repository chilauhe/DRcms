...
namespace DRcms.Web.admin.order
{
    public partial class order_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.status = Common.Request.GetQueryInt("status");
            this.payment_status = Common.Request.GetQueryInt("payment_status");
            this.express_status = Common.Request.GetQueryInt("express_status");
            this.keywords = Common.Request.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_list", Enums.ActionEnum.View.ToString()); //检查权限
                SiteBind(); //绑定站点
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("order_list_page_size", "DRcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("order_list_page_size", "DRcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("order_list.aspx", "site_id={0}&status={1}&payment_status={2}&express_status={3}&keywords={4}",
        ...
        {
            ChkAdminLevel("order_list", Enums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
            ...
            AddAdminLog(Enums.ActionEnum.Delete.ToString(), "删除订单成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
