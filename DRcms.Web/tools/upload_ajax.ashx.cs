...
namespace DRcms.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void UpLoadFile(HttpContext context)
        {
            ...
            string _delfile = Common.Request.GetString("DelFilePath"); //要删除的文件
            string fileName = Common.Request.GetString("name"); //文件名

            byte[] byteData = FileHelper.ConvertStreamToByteBuffer(context.Request.InputStream); //获取文件流
            bool _iswater = false; //默认不打水印
            bool _isthumbnail = false; //默认不生成缩略图

            if (Request.GetQueryString("IsWater") == "1")
            {
                _iswater = true;
            }
            if (Request.GetQueryString("IsThumbnail") == "1")
            {
                _isthumbnail = true;
            }
        ...
        {
            int Start = Common.Request.GetInt("start", 0); //开始索引
            int Size = Common.Request.GetInt("size", 20); //每页大小
            int Total = 0; //文件总数
