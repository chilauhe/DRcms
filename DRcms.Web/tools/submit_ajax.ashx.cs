...
namespace DRcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            //取得处事类型
            string action = Common.Request.GetQueryString("action");
        ...
        private void comment_add(HttpContext context)
        {
            ...
            string code = Common.Request.GetFormString("txtCode");
            int channel_id = Common.Request.GetQueryInt("channel_id");
            int article_id = Common.Request.GetQueryInt("article_id");
            string _content = Common.Request.GetFormString("txtContent");
            //校检验证码
            ...
            model.user_ip = Common.Request.GetIP();
        ...
        private void comment_list(HttpContext context)
        {
            int channel_id = Common.Request.GetQueryInt("channel_id");
            int article_id = Common.Request.GetQueryInt("article_id");
            int page_index = Common.Request.GetQueryInt("page_index");
            int page_size = Common.Request.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
        ...
        {
            string username = Common.Request.GetString("param");
            //如果为Null，退出
        ...
        {
            int site_id = Common.Request.GetQueryInt("site_id");
            string username = Common.Request.GetFormString("txtUserName");
            string password = Common.Request.GetFormString("txtPassword");
            string remember = Common.Request.GetFormString("chkRemember");
            //检查站点是否正确
            ...
            context.Session[Constants.SESSION_USER_INFO] = model;
            context.Session.Timeout = 45;
            //记住登录状态下次自动登录
            if (remember.ToLower() == "true")
            {
                Utils.WriteCookie(Constants.COOKIE_USER_NAME_REMEMBER, "DRcms", model.user_name, 43200);
                Utils.WriteCookie(Constants.COOKIE_USER_PWD_REMEMBER, "DRcms", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(Constants.COOKIE_USER_NAME_REMEMBER, "DRcms", model.user_name);
                Utils.WriteCookie(Constants.COOKIE_USER_PWD_REMEMBER, "DRcms", model.password);
            }
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = Common.Request.GetString("txtUserName");
            string password = Common.Request.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
        ...
        private void user_oauth_register(HttpContext context)
        {
            ...
            int site_id = Common.Request.GetQueryInt("site_id"); //当前站点ID
            //检查站点是否正确
            if (site_id == 0)
            {
            ...
            string password = Common.Request.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(Request.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(Request.GetFormString("txtMobile").Trim());
            string userip = Common.Request.GetIP();
            //反序列化JSON
            Dictionary<string, object> dic = JsonHelper.DataRowFromJSON(result);
            if (dic["ret"].ToString() != "0")
            {
            ...
            else if (userConfig.regmsgstatus == 2) //发送邮件
            {
                ...
                if (mailModel != null)
                {
                    ...
                    //发送邮件
                    Mail.send(sysConfig.emailsmtp, sysConfig.emailssl,
        ...
        {
            int site_id = Common.Request.GetQueryInt("site_id"); //当前站点ID
            string code = Common.Request.GetFormString("txtCode").Trim();
            string username = Utils.ToHtml(Request.GetFormString("txtUserName").Trim());
            string password = Common.Request.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(Request.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(Request.GetFormString("txtMobile").Trim());
            string userip = Common.Request.GetIP();
        ...
        {
            string mobile = Utils.ToHtml(Request.GetString("mobile"));
            //检查手机
        ...
        private void user_verify_email(HttpContext context)
        {
            ...
            string username = Utils.ToHtml(Request.GetFormString("username"));
            ...
            //检查邮件是否过快
            string cookie = Utils.GetCookie(Constants.COOKIE_USER_EMAIL);
            if (cookie == username)
            {
            ...
            Utils.WriteCookie(Constants.COOKIE_USER_EMAIL, username, 20); //20分钟内无重复发送
            return;
        }
        #endregion
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string nick_name = Utils.ToHtml(Request.GetFormString("txtNickName"));
            string sex = Common.Request.GetFormString("rblSex");
            string birthday = Common.Request.GetFormString("txtBirthday");
            string email = Utils.ToHtml(Request.GetFormString("txtEmail"));
            string mobile = Utils.ToHtml(Request.GetFormString("txtMobile"));
            string telphone = Utils.ToHtml(Request.GetFormString("txtTelphone"));
            string qq = Utils.ToHtml(Request.GetFormString("txtQQ"));
            string msn = Utils.ToHtml(Request.GetFormString("txtMsn"));
            string province = Utils.ToHtml(Request.GetFormString("txtProvince"));
            string city = Utils.ToHtml(Request.GetFormString("txtCity"));
            string area = Utils.ToHtml(Request.GetFormString("txtArea"));
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = Common.Request.GetFormString("hideFileName");
            int x1 = Common.Request.GetFormInt("hideX1");
            int y1 = Common.Request.GetFormInt("hideY1");
            int w = Common.Request.GetFormInt("hideWidth");
            int h = Common.Request.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = Common.Request.GetFormString("txtOldPassword");
            string password = Common.Request.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        private void user_getpassword(HttpContext context)
        {
            ...
            string sitepath = new BLL.sites().GetBuildPath(site_id);//站点目录
            string code = Common.Request.GetFormString("txtCode");
            string type = Common.Request.GetFormString("txtType");
            string username = Common.Request.GetFormString("txtUserName").Trim();
            //检查站点目录
            if (site_id == 0 || string.IsNullOrEmpty(sitepath))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，站点传输参数有误！\"}");
            ...
            if (type.ToLower() == "mobile") //使用手机取回密码
            {
                ...
                codeModel = codeBll.GetModel(username, Enums.CodeEnum.RegVerify.ToString(), "d");
                if (codeModel == null)
                {
                    ...
                    codeModel.type = Enums.CodeEnum.Password.ToString();
        ...
        {
            string strcode = Common.Request.GetFormString("hideCode"); //取回密码字符串
            string password = Common.Request.GetFormString("txtPassword"); //重设后的密码
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + Enums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + Enums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            string str_code = Utils.GetCheckCode(8);
            Model.user_code codeModel = new Model.user_code();
            codeModel.user_id = model.id;
            codeModel.user_name = model.user_name;
            codeModel.type = Enums.CodeEnum.Register.ToString();
            codeModel.str_code = str_code;
            codeModel.user_ip = Common.Request.GetIP();
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = Common.Request.GetFormInt("txtAmount");
            string password = Common.Request.GetFormString("txtPassword");
            if (model.amount < 1)
            {
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            int site_id = Common.Request.GetQueryInt("site_id"); //当前站点ID
            string sitepath = new BLL.sites().GetBuildPath(site_id); //站点目录
            decimal amount = Common.Request.GetFormDecimal("order_amount", 0);
            int payment_id = Common.Request.GetFormInt("payment_id");
            //检查站点目录
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string send_save = Common.Request.GetFormString("sendSave");
            string user_name = Utils.ToHtml(Request.GetFormString("txtUserName"));
            string title = Utils.ToHtml(Request.GetFormString("txtTitle"));
            string content = Utils.ToHtml(Request.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
        ...
        private void user_point_delete(HttpContext context)
        {
            ...
            string check_id = Common.Request.GetFormString("checkId");
        ...
        {
            int channel_id = Common.Request.GetInt("channel_id", 0);
            int article_id = Common.Request.GetFormInt("article_id", 0);
            int quantity = Common.Request.GetFormInt("quantity", 1);
        ...
        {
            string jsonData = Common.Request.GetFormString("jsondata");
            ...
            if (ls != null)
            {
                Utils.WriteCookie(Constants.COOKIE_SHOPPING_BUY, jsonData); //暂放在清单
        ...
        private void cart_goods_update(HttpContext context)
        {
            int channel_id = Common.Request.GetInt("channel_id", 0);
            int article_id = Common.Request.GetFormInt("article_id", 0);
            int quantity = Common.Request.GetFormInt("quantity", 0);
            if (channel_id == 0 || article_id == 0)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品参数有误！\"}");
                return;
        ...
        {
            int clear = Common.Request.GetFormInt("clear", 0);
            int channel_id = Common.Request.GetInt("channel_id", 0);
            int article_id = Common.Request.GetFormInt("article_id", 0);
            if (clear == 1)
            {
        ...
        {
            //获取传参信息===================================
            string hideGoodsJson = Utils.GetCookie(Constants.COOKIE_SHOPPING_BUY); //获取商品JSON数据
            int site_id = Common.Request.GetInt("site_id", 0); //站点ID
            int book_id = Common.Request.GetFormInt("book_id", 1);
            int payment_id = Common.Request.GetFormInt("payment_id");
            int express_id = Common.Request.GetFormInt("express_id");
            int is_invoice = Common.Request.GetFormInt("is_invoice", 0);
            string accept_name = Utils.ToHtml(Request.GetFormString("accept_name"));
            string province = Utils.ToHtml(Request.GetFormString("province"));
            string city = Utils.ToHtml(Request.GetFormString("city"));
            string area = Utils.ToHtml(Request.GetFormString("area"));
            string address = Utils.ToHtml(Request.GetFormString("address"));
            string telphone = Utils.ToHtml(Request.GetFormString("telphone"));
            string mobile = Utils.ToHtml(Request.GetFormString("mobile"));
            string email = Utils.ToHtml(Request.GetFormString("email"));
            string post_code = Utils.ToHtml(Request.GetFormString("post_code"));
            string message = Utils.ToHtml(Request.GetFormString("message"));
            string invoice_title = Utils.ToHtml(Request.GetFormString("invoice_title"));
            string sitepath = new BLL.sites().GetBuildPath(site_id); //站点目录
            ...
            //清空结账清单
            Utils.WriteCookie(Constants.COOKIE_SHOPPING_BUY, "");
            //提交成功，返回URL
            context.Response.Write("{\"status\":1, \"url\":\""
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = Common.Request.GetQueryString("order_no");
        ...
        {
            int channel_id = Common.Request.GetInt("channel_id", 0);
            int article_id = Common.Request.GetInt("id", 0);
            int click = Common.Request.GetInt("click", 0);
            int hide = Common.Request.GetInt("hide", 0);
            int count = 0;
            if (channel_id > 0 && article_id > 0)
            {
        ...
        {
            int channel_id = Common.Request.GetInt("channel_id", 0); //频道ID
            int id = Common.Request.GetInt("id", 0); //附件ID或文章ID
            string view = Common.Request.GetString("view");
            int count = 0;
            if (id > 0)
            {
        ...
        private string verify_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[Constants.SESSION_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[Constants.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[Constants.SESSION_CODE] = null;
            return "success";
        }
        #endregion
        ...
        private string verify_sms_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[Constants.SESSION_SMS_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[Constants.SESSION_SMS_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[Constants.SESSION_SMS_CODE] = null;
            return "success";
        }
        #endregion
        ...
        private string send_verify_sms_code(HttpContext context,string mobile)
        {
            ...
            //检查是否过期
            string cookie = Utils.GetCookie(Constants.COOKIE_USER_MOBILE);
            if (cookie == mobile)
            {
            ...
            //写入SESSION，保存验证码
            context.Session[Constants.SESSION_SMS_CODE] = strcode;
            Utils.WriteCookie(Constants.COOKIE_USER_MOBILE, mobile, userConfig.regsmsexpired); //规定时间内无重复发送
            return "success";
        }
        #endregion
        ...
        private string send_verify_email(string sitepath, Model.users userModel)
        {
            ...
            Model.user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, Enums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = Enums.CodeEnum.RegVerify.ToString();
                codeModel.str_code = strcode;
                codeModel.eff_time = DateTime.Now.AddDays(userConfig.regemailexpired);
                codeModel.add_time = DateTime.Now;
